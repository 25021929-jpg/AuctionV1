...
public class WalletController {
    ...
    public Response<WalletSummaryDto> deposit(String requestBody) {
        try {
            ...
            WalletSummaryDto summary = walletService.deposit(request == null ? null : request.getUserId(),
    ...
    public Response<List<WalletTransactionDto>> getTransactions(String requestBody) {
        try {
            WalletTransactionHistoryRequest request = gson.fromJson(requestBody, WalletTransactionHistoryRequest.class);
            Long userId = request == null ? null : request.getUserId();
            int limit = request == null ? 50 : request.getLimit();
            return Response.success("Wallet transactions loaded", walletService.getTransactions(userId, limit));
        } catch (Exception e) {
            return Response.fail(e.getMessage());
        }
    }
