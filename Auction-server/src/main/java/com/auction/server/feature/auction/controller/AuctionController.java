 ...
 *
 * <p>Trách nhiệm duy nhất: - Parse JSON body → DTO - Gọi Service - Bắt exception → trả
 * Response.fail() có message - Không chứa logic nghiệp vụ
 *
 * <p>Exception strategy (đơn giản hóa): - AuctionException → lỗi nghiệp vụ → trả message cho client
 * - Exception → lỗi hệ thống → trả message chung Không dùng BusinessException hierarchy để giữ code
 * đơn giản.
 */
public class AuctionController {
  ...
  /** Lấy danh sách phiên đấu giá đang hoạt động. Request body: { "page": 0, "size": 20 } */
  ...
  /**
   * Lấy danh mục sản phẩm từ bảng categories trong database. Client dùng kết quả này để hiển thị
   * ComboBox tên danh mục thay vì bắt nhập categoryId.
   */
  ...
  /** Lấy chi tiết 1 phiên đấu giá. Request body: { "auctionId": 42 } */
  ...
  /** Tạo phiên đấu giá mới. Request body: JSON của CreateAuctionRequest */
