 ...
 *
 * <p>Nguyên tắc Service Layer: - Chỉ chứa Business Logic — không chứa SQL, Hibernate, HTTP - Nhận
 * DTO từ tầng trên, trả DTO về tầng trên - Không biết Session tồn tại — DbExecutor lo transaction -
 * Phối hợp nhiều Repository trong 1 usecase - Quyết định khi nào cần lock (findByIdWithLock)
 *
 * <p>Dependency Injection: Service nhận Interface qua constructor. Không biết implementation nào
 * đang dùng. Main.java tạo implementation và inject vào. → DIP (Dependency Inversion Principle) →
...
public class BidService {
   ...
   *
   * <p>Tại sao không có constructor mặc định tạo new HibernateXxx()? Constructor mặc định buộc
   * Service phải biết HibernateXxx tồn tại. Vi phạm DIP: tầng cao (Service) phụ thuộc tầng thấp
   * (HibernateXxx). Khó test: không thể inject MockRepository. Main.java mới là nơi tạo
   * implementation và inject.
   */
  public BidService(
  ...
  public BidService() {
    this(
        new HibernateAuctionSessionRepository(
            com.auction.server.database.HibernateUtil.getSessionFactory()),
        ...
        new HibernateWalletTransactionRepository(
            com.auction.server.database.HibernateUtil.getSessionFactory()));
  }
  ...
  public BidResponse placeBid(PlaceBidRequest request) {
    ...
    return DbExecutor.runAndReturn(
        () -> {
          ...
          // 2. BẮT BUỘC dùng findByIdWithLock (PESSIMISTIC_WRITE) để khóa dòng dữ liệu của phiên
          // đấu giá này lại.
          ...
          User bidder =
              userRepository
                  .findByIdWithLock(request.getBidderId())
  ...
  public List<BidResponse> getBidHistory(Long auctionId, int limit) {
    ...
    return DbExecutor.query(
        () -> {
          ...
          return topBids.stream().map(this::toResponse).collect(Collectors.toList());
        });
  }
  ...
  /**
   * Cập nhật trạng thái ngay trong transaction đặt giá. Đây là lớp phòng thủ cuối cùng để phiên đã
   * tới giờ có thể nhận bid ngay cả khi scheduler chưa chạy hoặc client vừa tạo phiên xong.
   */
   ...
   *
   * <p>Validate ở đây: - null check - range check (> 0) - format check (BigDecimal không âm)
   *
   * <p>Validate KHÔNG ở đây (thuộc Business Logic trong transaction): - Auction có active không →
   * cần đọc DB có lock - Giá có hợp lệ không → cần đọc currentPrice có lock - User có tồn tại không
   * → cần đọc DB
   */
  private void validatePlaceBidRequest(PlaceBidRequest request) {
    ...
    if (request.getAuctionSessionId() == null || request.getAuctionSessionId() <= 0) {
      throw new BidException("AuctionSessionId không hợp lệ");
    }
    if (request.getBidderId() == null || request.getBidderId() <= 0) {
