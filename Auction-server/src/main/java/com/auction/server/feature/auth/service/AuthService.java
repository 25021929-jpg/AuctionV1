 ...
 *
 * <p>Nguyên tắc thiết kế: - Không biết về Session/Transaction — DbExecutor lo phần đó - Không lộ
 * passwordHash ra ngoài — dùng UserInfo DTO - Không tiết lộ thông tin nhạy cảm trong message lỗi -
 * Validate format trước khi mở transaction → tránh tốn DB connection khi input sai
 *
 * <p>Tối ưu connection pool: - Bcrypt (~100ms CPU) được đưa RA NGOÀI transaction - DB connection
 * chỉ bị giữ khi thực sự cần query (~2-4ms)
 */
public class AuthService {
  ...
  /**
   * Luồng xử lý nguyên tử (Atomic Register): 1. Validate format & Chuẩn hóa (Ngoài TX) 2. Hash
   * password thô (Ngoài TX - Tốn 100ms CPU nhưng Connection Pool rảnh hoàn toàn) 3. [TX Duy Nhất]
   * Kiểm tra trùng + Lưu DB (Connection giữ ~2-3ms rồi đóng ngay)
   */
   ...
   *
   * <p>Luồng (tối ưu connection pool — hot path): 1. validate format — ngoài transaction 2. [TX
   * read-only] tìm user — connection giữ ~1ms rồi trả về pool 3. verifyPassword() — NGOÀI
   * transaction, CPU ~100ms, DB connection rảnh
   *
   * <p>Bảo mật: - Không phân biệt "sai username" hay "sai password" → chống Account Enumeration
   * Attack
   *
  ...
  /**
   * Validate format request đăng ký. Gọi TRƯỚC transaction — không tốn DB connection nếu input sai.
   */
  ...
  /**
   * Chỉ cho phép người dùng tự đăng ký vai trò nghiệp vụ thông thường. ADMIN không được tạo từ form
   * đăng ký public.
   */
  ...
  /**
   * Convert User entity → UserInfo DTO an toàn để trả về client. User entity chứa passwordHash —
   * không bao giờ được ra ngoài Service.
   */
