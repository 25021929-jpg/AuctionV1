 ...
 *
 * <p>Although seller screens talk about "items", an item being sold in this app is represented by
 * two database concepts: {@link AuctionItem} for product metadata and {@link AuctionSession} for
 * price/time/status. Keeping that orchestration here avoids pushing seller-specific rules into the
...
public class SellerService {
  ...
  public List<SellerItemDto> listMyItems(long sellerId, int page, int size) {
    ...
    return DbExecutor.query(
        () -> {
          ...
          return auctionSessionRepository.findBySeller(sellerId, page, size).stream()
              .map(this::toDto)
              .toList();
        });
  }
  ...
  public SellerItemDto createItem(CreateSellerItemRequest request) {
    ...
    return DbExecutor.runAndReturn(
        () -> {
          categoryRepository
              .findById(toCategoryId(request.getCategoryId()))
              .orElseThrow(
                  () -> new SellerException("Category not found: " + request.getCategoryId()));
          ...
          Category category =
              categoryRepository.getReference(toCategoryId(request.getCategoryId()));
    ...
    return DbExecutor.runAndReturn(
        () -> {
          ...
          AuctionSession auction =
              findOwnedAuction(request.getSellerId(), request.getItemId(), request.getAuctionId());
  ...
  public void deleteItem(DeleteSellerItemRequest request) {
    ...
    DbExecutor.run(
        () -> {
          ...
          // Soft delete: keep historical consistency for bids/payments and hide it from seller
          // workflows.
   ...
   *
   * <ul>
   *   <li>startTime &lt;= now và endTime &gt; now: phiên phải ACTIVE.
   *   <li>endTime &lt;= now: không xử lý ở đây, để scheduler chốt ENDED kèm ví.
   * </ul>
   *
    ...
    DbExecutor.runAndReturn(
        () -> {
          ...
          List<Long> shouldStart =
              auctionSessionRepository.findScheduledToStart().stream()
                  .filter(
                      auction -> auction.getEndTime() != null && auction.getEndTime().isAfter(now))
  ...
  private void validateCreate(CreateSellerItemRequest request) {
    ...
    if (request.getStartPrice() == null
        || request.getStartPrice().compareTo(BigDecimal.ZERO) <= 0) {
