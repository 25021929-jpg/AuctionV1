 ...
 *
 * Payment là bảng nhạy cảm nhất — liên quan đến tiền thật.
 * Mọi thao tác ghi phải nằm trong transaction.
 */
public interface PaymentRepository {
    ...
    /**
     * Tìm payment của một phiên đấu giá.
     * 1 auction = tối đa 1 payment (UNIQUE constraint).
     */
    ...
    /**
     * Lịch sử bán hàng của seller — chỉ COMPLETED.
     * Dùng cho: trang "Doanh thu của tôi".
     */
    List<Payment> findBySeller(Long sellerId, int page, int size);

    /**
     * Tìm payment PENDING quá lâu.
     * Dùng cho: cron job xử lý timeout payment.
     */
    List<Payment> findPendingOlderThan(LocalDateTime threshold);

    /**
     * Tổng doanh thu thực nhận của seller (sau phí sàn).
     * Trả BigDecimal.ZERO nếu chưa có payment nào.
     */
     ...
     * Dùng khi cổng thanh toán callback.
     * Trả boolean để biết có cập nhật được không.
     */
    boolean updateStatus(Long paymentId,
