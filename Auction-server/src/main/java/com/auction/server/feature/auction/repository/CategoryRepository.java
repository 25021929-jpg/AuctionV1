 ...
 *
 * <p>Category có tính chất đặc biệt: self-referencing (cha-con). Một category có thể có parent
 * (danh mục cha) và danh sách children (danh mục con).
 *
 * <p>Ví dụ: Điện tử (gốc) └── Điện thoại (con) └── iPhone (cháu)
 */
public interface CategoryRepository {
  ...
  /** Lấy danh mục con trực tiếp của một cha. Dùng cho: render menu dropdown. */
  List<Category> findChildren(Integer parentId);

  /**
   * Lấy toàn bộ cây danh mục — dùng cho menu navigation. Load tất cả trong 1 query, tránh N+1 khi
   * render cây.
   */
