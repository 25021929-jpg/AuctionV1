...
/**
 * Implementation dùng Hibernate. Repository chỉ lo query — không quản lý transaction. Session lấy
 * qua getCurrentSession() — Session của Thread hiện tại. Lock được thiết lập trên Session đó — sống
 * theo transaction của Service.
 */
public class HibernateAuctionSessionRepository implements AuctionSessionRepository {
  ...
  public Optional<AuctionSession> findById(Long id) {
    ...
    return Optional.ofNullable(sessionFactory.getCurrentSession().get(AuctionSession.class, id));
  }
  ...
  @Override
  public List<AuctionSession> findBySeller(Long sellerId, int page, int size) {
    return sessionFactory
        .getCurrentSession()
        .createQuery(
            """
  ...
  public Optional<AuctionSession> findByIdWithLock(Long id) {
    ...
    return Optional.ofNullable(
        sessionFactory
            .getCurrentSession()
            .get(AuctionSession.class, id, LockMode.PESSIMISTIC_WRITE));
  }
  ...
  public AuctionSession save(AuctionSession auction) {
    return (AuctionSession) sessionFactory.getCurrentSession().merge(auction);
    // merge(): INSERT nếu mới, UPDATE nếu đã có id
    // Dirty Checking: nếu entity đang Persistent → tự UPDATE khi flush
  }

  @Override
  public int bulkUpdateStatus(List<Long> ids, AuctionSession.AuctionStatus status) {
    if (ids == null || ids.isEmpty()) return 0;
    return sessionFactory
        .getCurrentSession()
        .createMutationQuery(
            "UPDATE AuctionSession a SET a.status = :status " + "WHERE a.auctionId IN :ids")
