 ...
 *
 * <p>Nguyên tắc thiết kế: - Không biết về Session/Transaction — DbExecutor lo phần đó - Không tự
 * new repository — nhận qua constructor (DIP) - validate format trước transaction — không tốn DB
 * connection khi input sai - toDto() map bên trong session — tránh LazyInitializationException
 *
 * <p>Exception strategy (đơn giản hóa): - Chỉ dùng AuctionException cho mọi lỗi nghiệp vụ - Không
 * dùng DataNotFoundException/DuplicateDataException hierarchy - DbExecutor tự ném DatabaseException
 * nếu lỗi hạ tầng — không cần catch thêm
 *
 * <p>NOTE: Do not instantiate repository implementations here. Use constructor injection from
 * composition root (Main) to preserve DIP.
 */
public class AuctionService {
   ...
   *
   * <p>Dùng DbExecutor.query() — read-only, tắt dirty checking. Không catch DataAccessException —
   * DbExecutor tự xử lý lỗi hạ tầng.
   */
  public List<AuctionResponse> getAllAuctions(int page, int size) {
    ...
    return DbExecutor.query(
        () ->
            auctionSessionRepository.findActive(page, size).stream()
   ...
   * categoryId/categoryName ở giao diện vì dữ liệu thật nằm ở database.
   */
  public List<CategoryDto> listCategories() {
    return DbExecutor.query(
        () -> categoryRepository.findAllSorted().stream().map(this::toCategoryDto).toList());
  }
    ...
    return DbExecutor.query(
        () ->
            auctionSessionRepository
                ...
                .orElseThrow(() -> new AuctionException("Auction not found: id = " + auctionId)));
  }
   ...
   *
   * <p>Luồng: 1. validate format — ngoài transaction 2. [TX] kiểm tra category + tạo AuctionItem +
   * tạo AuctionSession — atomic → nếu save AuctionSession lỗi thì AuctionItem cũng rollback
   *
    ...
    return DbExecutor.runAndReturn(
        () -> {
          ...
          categoryRepository
              ...
              .orElseThrow(
                  () ->
                      new AuctionException("Category not found: id = " + request.getCategoryId()));
   ...
   *
   * <p>Dự án hiện không có scheduler chạy nền ổn định, nên nếu không làm bước này phiên đã tới giờ
   * vẫn nằm SCHEDULED và người dùng không thể đặt giá.
   */
  private void refreshAuctionStatuses() {
    DbExecutor.runAndReturn(
        () -> {
          ...
          List<Long> shouldStart =
              scheduledToStart.stream()
                  .filter(a -> a.getEndTime() != null && a.getEndTime().isAfter(now))
                  .map(AuctionSession::getAuctionId)
                  .toList();
          if (!shouldStart.isEmpty()) {
            auctionSessionRepository.bulkUpdateStatus(
                shouldStart, AuctionSession.AuctionStatus.ACTIVE);
          }
  ...
  private AuctionSession.AuctionStatus initialAuctionStatus(
  ...
  /**
   * Map → AuctionResponse (tóm tắt, dùng cho danh sách). PHẢI gọi trong lambda DbExecutor — session
   * còn mở để access lazy relation.
   */
  ...
  /**
   * Map → AuctionDetailResponse (đầy đủ, dùng cho trang chi tiết). PHẢI gọi trong lambda DbExecutor
   * — session còn mở để access lazy relation.
   */
