 ...
 *
 * <p>Lưu ý quan trọng về Cascade: AuctionItem có cascade = ALL đến ItemImage. → save(item) tự save
 * images. → delete(item) tự delete images. Repository này chỉ cần thiết khi thao tác ảnh ĐỘC LẬP
 * (thêm/xoá ảnh sau khi item đã tạo).
 */
public class HibernateItemImageRepository implements ItemImageRepository {
  ...
  /**
   * Tất cả ảnh của item — sắp xếp theo sort_order. sort_order = 0 (ảnh chính thường là 0) lên đầu.
   */
  @Override
   ...
   *
   * <p>Trả Optional vì item có thể chưa có ảnh nào.
   */
  @Override
  ...
  public ItemImage save(ItemImage image) {
    return (ItemImage) sessionFactory.getCurrentSession().merge(image);
  }

  /**
   * Xoá ảnh theo id.
   *
   * <p>Tại sao load entity trước rồi mới remove? session.remove() cần Persistent entity — không
   * dùng được id trực tiếp. Nếu không tìm thấy → không làm gì (idempotent).
   */
  @Override
  public void deleteById(Long imageId) {
    sessionFactory
        .getCurrentSession()
        .createMutationQuery("DELETE FROM ItemImage img WHERE img.id = :id")
   ...
   *
   * <p>Tại sao phải 2 bước? Bảng có thể có nhiều ảnh is_primary = true nếu không reset. Bước 1
   * reset tất cả → Bước 2 set đúng 1 ảnh. 2 bước này PHẢI trong cùng 1 transaction của Service. Nếu
   * bước 2 lỗi → bước 1 rollback → không có ảnh nào là primary.
   *
   * <p>Tại sao không trả boolean? Nếu imageId không tồn tại → bước 2 update 0 row. Service tự kiểm
   * tra bằng findPrimaryImage() sau đó nếu cần.
   */
  @Override
