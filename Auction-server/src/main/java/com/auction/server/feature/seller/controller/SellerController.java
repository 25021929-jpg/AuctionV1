 ...
 *
 * <p>The dispatcher passes only WireMessage.data as raw JSON. This controller parses that JSON into
 * shared DTOs, delegates business rules to SellerService, then returns Response for ClientHandler
 * to wrap back into a WireMessage response.
 */
  ...
  public Response<List<SellerItemDto>> listMyItems(String requestBody) {
    try {
      ...
      return Response.success(
          "Seller items loaded", sellerService.listMyItems(sellerId, page, size));
