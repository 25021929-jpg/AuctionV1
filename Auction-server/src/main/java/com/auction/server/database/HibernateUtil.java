 ...
 * nhiệm vụ của nó:
 * +
 * Nói rõ đây là provider — cung cấp session
 * +
 * Thể hiện đây là infrastructure quan trọng
 * +
 * Lifecycle rõ ràng: init → use → destroy
 * // Tên nói lên đây là tầng persistence // Không phải helper function đơn giản
 * */
public class HibernateUtil {
    ...
    public static synchronized void initialize(HikariDataSource dataSource) {
        // Khóa bảo vệ (Guard Clause): Nếu đã khởi tạo rồi thì bỏ qua, tránh tạo đè gây rò rỉ (leak) vùng nhớ
    ...
    /**
     * Mở một phiên làm việc mới (Session) với Database.
     * Mỗi Thread xử lý Request từ Client nên mở một Session riêng và đóng lại ngay khi dùng xong.
     */
    public static Session openSession() {
        if (sessionFactory == null) {
            throw new IllegalStateException("Cảnh báo: Bạn phải gọi HibernateUtil.initialize() trước khi mở Session!");
        }
        return sessionFactory.openSession();
    }

    /**
     * Đóng nhà máy SessionFactory khi Server tắt (Shutdown hook).
     * Giải phóng toàn bộ bộ nhớ đệm (Cache) và các tài nguyên nội bộ mà Hibernate nắm giữ.
     */
