 ...
 *
 * <ol>
 *   <li>{@link #initializePool()} — gọi <b>một lần</b> khi server khởi động (MainServer)
 *   <li>{@link #getConnection()} — repository mượn Connection, trả bằng {@code close()} trong
 *       try-with-resources
 *   <li>{@link #shutdownPool()} — gọi khi tắt server (shutdown hook)
 * </ol>
 *
 * {@code conn.close()} trên connection từ pool <b>không</b> đóng TCP — Hikari trả connection về
 * pool (xem tài liệu HikariProxyConnection trong giaithich.md).
 */
public class DatabaseConnection {
   ...
   *
   * <p>Thread-safe: {@code synchronized} + kiểm tra {@code dataSource != null} tránh tạo hai pool
  ...
  public static synchronized void initializePool() {
    ...
    // 2. XÓA BỎ setConnectionTestQuery("SELECT 1") để dùng Connection.isValid() mặc định cực
    // nhanh.(của hikariCP, không phải của MySQL driver) — tránh query thừa mỗi lần mượn connection.
    ...
    config.addDataSourceProperty(
        "prepStmtCacheSqlLimit", "2048"); // Giới hạn độ dài câu SQL được cache
    config.addDataSourceProperty(
        "useServerPrepStmts",
        "true"); // BẮT BUỘC: Ép MySQL biên dịch trước để chống SQL Injection và tăng tốc
  ...
  /**
   * Mượn một {@link Connection} từ pool. Bắt buộc đóng trong try-with-resources — Hikari nhận lại
   * connection, không tạo socket mới mỗi lần gọi.
   *
  ...
  /**
   * Đọc {@value #PROPERTIES_FILE} qua ClassLoader của process server. File nằm tại {@code
   * src/main/resources/} → sau build: {@code target/classes/}.
   */
  private static Properties loadPropertiesFromClasspath() {
    Properties props = new Properties();
    try (InputStream is =
        DatabaseConnection.class.getClassLoader().getResourceAsStream(PROPERTIES_FILE)) {
