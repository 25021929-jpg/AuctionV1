 ...
 *
 * Phạm vi test:
 *  - placeBid():   validate PlaceBidRequest trước khi vào transaction
 *  - getBidHistory(): validate auctionId và giới hạn limit
 *
 * Kiến trúc test:
 *  Validate của BidService nằm trong validatePlaceBidRequest() — gọi TRƯỚC DbExecutor.
 *  → Có thể test độc lập mà không cần Hibernate Session.
 *
 * Các trường hợp Business Logic trong transaction (auction không active,
 * giá không hợp lệ...) cần Integration Test với H2 hoặc mockStatic DbExecutor.
 */
...
class BidServiceTest {
    ...
    class PlaceBidRequestDtoTest {
        ...
        void setters_overrideInitialValues() {
            ...
            org.assertj.core.api.Assertions.assertThat(req.getBidAmount())
