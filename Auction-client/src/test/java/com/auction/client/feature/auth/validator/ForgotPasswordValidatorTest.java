...
public class ForgotPasswordValidatorTest {
  ...
  void validate_validEmail_returnsOk() {
    ValidationResult result = validator.validate(new ForgotPasswordRequest("john@gmail.com"));
  ...
  void validate_emptyEmail_returnsError() {
    ValidationResult result = validator.validate(new ForgotPasswordRequest(""));
    assertFalse(result.valid());
    assertTrue(result.hasErrorFor("email"));
    assertEquals("Thành phần không được để trống", result.errorFor("email"));
  }

  @Test
  @DisplayName("Sai định dạng email -> error")
  void validate_invalidEmail_returnsError() {
    ValidationResult result = validator.validate(new ForgotPasswordRequest("john@"));
    assertFalse(result.valid());
    assertTrue(result.hasErrorFor("email"));
  ...
  void validate_nullEmail_returnsError() {
    ValidationResult result = validator.validate(new ForgotPasswordRequest(null));
    assertFalse(result.valid());
    assertTrue(result.hasErrorFor("email"));
  }
}
