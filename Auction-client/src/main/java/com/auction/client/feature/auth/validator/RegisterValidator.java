...
public class RegisterValidator implements Validator<RegisterRequest> {
  ...
  public ValidationResult validate(RegisterRequest req) {
    ...
    validate("fullName", req.fullName(), new NotBlankRule(), new FullNameRule())
    ...
    validate("password", req.password(), new NotBlankRule(), new MinLengthRule(8))
        .ifPresent(errors::add);

    validate("email", req.email(), new NotBlankRule(), new EmailFormatRule())
        .ifPresent(errors::add);

    validate("phoneNumber", req.phoneNumber(), new NotBlankRule(), new PhoneNumberRule())
        .ifPresent(errors::add);

    validate("birthDate", req.birthDate(), new MinAgeRule(18)).ifPresent(errors::add);
    ...
    if (!errors.stream().anyMatch(e -> e.field().equals("password"))) {
      if (!req.password().equals(req.confirmPassword())) {
        errors.add(new FieldError("confirmPassword", "Mật khẩu không khớp"));
      }
    }
