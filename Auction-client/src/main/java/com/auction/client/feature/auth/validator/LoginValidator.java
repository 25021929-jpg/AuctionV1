...
public class LoginValidator implements Validator<LoginRequest> {
  ...
  public ValidationResult validate(LoginRequest req) {
    ...
    validate("password", req.password(), new NotBlankRule()).ifPresent(errors::add);

    return errors.isEmpty() ? ValidationResult.ok() : ValidationResult.from(errors);
  }

  @SafeVarargs
  // Helper method validate để giúp cho validate các thuộc tính thuận tiện hơn
  private static <T> Optional<FieldError> validate(
      String fieldName, T value, ValidationRule<T>... rules) {
