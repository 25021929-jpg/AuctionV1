 ...
 *
 * <p>Nguyên tắc: - KHÔNG "đoán" schema cứng. Client parse theo kiểu tolerant: + chấp nhận nhiều key
 * phổ biến (auctionId/id, itemName/name, currentPrice/currentBid...) + nếu thiếu field => set
 * default và UI vẫn chạy. - Khi team server chốt schema, chỉ cần sửa mapping trong class này.
 */
public class AuctionServiceImpl implements AuctionService {
  ...
  public List<AuctionSummaryDto> fetchAuctions() throws IOException {
    Response<JsonElement> res =
        SocketClient.getInstance().send(ActionConstants.AUCTION_GET_LIST, null, JsonElement.class);
  ...
  public AuctionDetailDto fetchAuctionDetail(long auctionId) throws IOException {
    ...
    if (data == null || !data.isJsonObject()) {
      throw new ApiException(
          ActionConstants.AUCTION_GET_DETAIL, "Dữ liệu chi tiết phiên đấu giá không hợp lệ");
    }
