...
public class SellerDashboardController implements Initializable, DisposableController {
  ...
  private final ObservableList<CategoryOption> categoryOptions =
  ...
  public void initialize(URL location, ResourceBundle resources) {
    try {
      AccessGuard.requireAnyRole(
          com.auction.shared.domain.UserRole.SELLER, com.auction.shared.domain.UserRole.ADMIN);
    } catch (Exception ex) {
    ...
    btnEdit
        .disableProperty()
        .bind(loading.or(tblItems.getSelectionModel().selectedItemProperty().isNull()));
    btnDelete
        .disableProperty()
        .bind(loading.or(tblItems.getSelectionModel().selectedItemProperty().isNull()));
  ...
  /**
   * Seller Dashboard không đứng trong từng auction room như màn Live Bidding, nên không thể chỉ phụ
   * thuộc vào nút Refresh. Timer này tự kiểm tra các mốc start/end đang hiển thị và gọi lại server
   * khi đến thời điểm cần đổi trạng thái.
   */
  ...
  private void refreshWhenVisibleStatusBoundaryReached() {
    ...
    boolean shouldRefresh =
        tblItems.getItems().stream().anyMatch(item -> isStatusBoundaryReached(item, now));
    if (shouldRefresh) {
      loadMyItemsAsync();
    }
  }
  ...
  public void handleAdd() {
    ...
    created.ifPresent(
        item -> runMutation(() -> sellerService.createItem(item), "Đã tạo sản phẩm."));
  }
  ...
  public void handleDelete() {
    ...
    confirm.setContentText(
        "Hệ thống sẽ chuyển phiên sang trạng thái CANCELLED, không xóa cứng dữ liệu để giữ lịch sử.");
  ...
  private void loadCategoriesAsync() {
    ...
    FxAsync.run(
        () -> {
          try {
            return sellerService.listCategories().stream().map(this::toCategoryOption).toList();
          } catch (Exception ex) {
            throw new RuntimeException(ex);
          }
        },
  ...
  private Optional<SellerItemDto> showItemFormDialog(SellerItemDto existing) {
    ...
    dialog.setHeaderText(
        existing == null ? "Nhập thông tin sản phẩm" : "Cập nhật thông tin sản phẩm");
    ...
    if (categoryOptions.isEmpty()) {
      AlertHelper.showError(
          "Chưa có danh mục",
          "Loại sản phẩm chưa tải được từ server/database. Hãy bấm Refresh hoặc kiểm tra bảng categories.");
    ...
    TextField tfStartPrice =
        new TextField(existing == null ? "" : MoneyFormat.plain(existing.getStartPrice()));
    ...
    Label timeHint =
        new Label(
            "Định dạng giờ: dd/MM/yyyy HH:mm. Ví dụ: 31/05/2026 20:30. Giờ bắt đầu phải trước giờ kết thúc; giờ kết thúc phải lớn hơn hiện tại. Nếu giờ bắt đầu đã qua nhưng giờ kết thúc còn ở tương lai, phiên sẽ được mở ACTIVE ngay.");
    ...
    okButton.addEventFilter(
        ...
        event -> {
          if (!validateSellerForm(
              tfName, taDesc, cbCategory, tfStartPrice, tfStartTime, tfEndTime, existing == null)) {
            /*
  ...
  private boolean validateSellerForm(
      TextField tfName,
      TextArea taDesc,
  ...
  private void selectCategory(ComboBox<CategoryOption> comboBox, long categoryId) {
    ...
    comboBox.getItems().stream()
        ...
        .findFirst()
        .ifPresentOrElse(
            comboBox.getSelectionModel()::select, () -> comboBox.getSelectionModel().selectFirst());
  }
