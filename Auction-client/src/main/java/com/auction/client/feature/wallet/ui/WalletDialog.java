...
public final class WalletDialog {
    ...
    public static void showWallet() {
        try {
            ...
            dialog.setHeaderText("Số dư hiện tại: " + money(summary == null ? UserSession.getInstance().getBalance() : summary.getBalance()));
            ...
            Label hint = new Label("Tiền nạp phải là số nguyên dương. Không chấp nhận 0, số âm hoặc tiền lẻ.");
            ...
            depositButton.addEventFilter(javafx.event.ActionEvent.ACTION, event -> {
                event.consume();
    ...
    public static boolean showDepositDialog() {
        ...
        if (amount.stripTrailingZeros().scale() > 0) {
            AlertHelper.showError("Số tiền không hợp lệ", "Số tiền nạp phải là số nguyên, không nhập tiền lẻ.");
            return false;
        }
    ...
    private static TableView<WalletTransactionDto> createHistoryTable(List<WalletTransactionDto> rows) {
        ...
        timeCol.setCellValueFactory(cell -> new ReadOnlyStringWrapper(
                cell.getValue().getCreatedAt() == null ? "-" : TIME_FMT.format(cell.getValue().getCreatedAt())));

        TableColumn<WalletTransactionDto, String> typeCol = new TableColumn<>("Loại");
        typeCol.setPrefWidth(140);
        typeCol.setCellValueFactory(cell -> new ReadOnlyStringWrapper(typeText(cell.getValue().getType())));

        TableColumn<WalletTransactionDto, String> amountCol = new TableColumn<>("Số tiền");
        amountCol.setPrefWidth(130);
        amountCol.setCellValueFactory(cell -> new ReadOnlyStringWrapper(money(cell.getValue().getAmount())));

        TableColumn<WalletTransactionDto, String> balanceCol = new TableColumn<>("Số dư sau GD");
        balanceCol.setPrefWidth(130);
        balanceCol.setCellValueFactory(cell -> new ReadOnlyStringWrapper(money(cell.getValue().getBalanceAfter())));
