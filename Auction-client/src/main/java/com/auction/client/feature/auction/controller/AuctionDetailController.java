 ...
 *
 * <p>View gọi {@link AuctionService} để tải dữ liệu chi tiết từ server. Nếu server chưa hỗ trợ đầy
 * đủ field, controller sẽ hiển thị theo dữ liệu nhận được.
 */
    ...
    FxAsync.run(
        ...
        dto -> {
          ...
          if (lblStartPrice != null)
            lblStartPrice.setText(MoneyFormat.grouped(dto.getStartingPrice()));
          ...
          if (lblStartTime != null) {
            lblStartTime.setText(
                dto.getStartTime() == null ? "-" : DT_FMT.format(dto.getStartTime()));
          }
  ...
  public void handleGoToLiveBidding() {
    SceneNavigator.switchScene(
        ...
        controller -> {
          ...
          if (controller
              instanceof com.auction.client.feature.bidding.controller.LiveBiddingController c) {
