 ...
 *
 * <p>Realtime update: nhận EVENT_BID_UPDATED từ server (Socket push) và cập nhật UI.
 */
public class LiveBiddingController implements DisposableController {
   ...
   *
   * <p>Dùng để validate tối thiểu phía client: bid phải lớn hơn giá hiện tại. Server vẫn là nơi
   * kiểm tra cuối cùng.
   */
  ...
  private void loadInitialDetail() {
    ...
    FxAsync.run(
        ...
        err -> {
          // Nếu không load được detail thì vẫn cho phép xem chart; việc bid sẽ do server quyết
          // định.
  ...
  private void setupBidHistoryTable() {
    ...
    colBidAmount.setCellValueFactory(
        cell -> new ReadOnlyStringWrapper(cell.getValue().amountText()));
    colBidWinning.setCellValueFactory(
        cell -> new ReadOnlyStringWrapper(cell.getValue().winningText()));
  ...
  private void applyBidHistory(List<BidResultDto> history) {
    ...
    Platform.runLater(
        () -> {
          bidHistoryRows.setAll(
              history.stream()
                  .sorted(
                      Comparator.comparing(
                          BidResultDto::getBidTime,
          ...
          history.stream()
              ...
              .sorted(
                  Comparator.comparing(
                      BidResultDto::getBidTime, Comparator.nullsLast(Comparator.naturalOrder())))
  ...
  private void applyDetail(AuctionDetailDto dto) {
    ...
    if (lblStartTime != null) {
      lblStartTime.setText(
          dto.getStartTime() == null ? "-" : DETAIL_TIME_FMT.format(dto.getStartTime()));
    }
    if (lblEndTime != null) {
  ...
  private void onAuctionStatusChangedEvent(AppEvent event) {
    ...
    Platform.runLater(
        () -> {
          ...
          if (lblAuctionStatus != null)
            lblAuctionStatus.setText(finalStatus == null ? "-" : finalStatus.name());
          updateBiddingAvailability();
  ...
  private void onBidUpdatedEvent(AppEvent event) {
    ...
    String x =
        (ts != null) ? TIME_FMT.format(Instant.ofEpochMilli(ts)) : TIME_FMT.format(Instant.now());
  ...
  private BidResultDto toBidResultDto(
      JsonObject data, Long eventAuctionId, BigDecimal currentPrice) {
  ...
  private static String formatBidTimeForChart(LocalDateTime value) {
    return value == null
        ...
        : value.toLocalTime().format(DateTimeFormatter.ofPattern("HH:mm:ss"));
  }
  ...
  public void handlePlaceBid() {
    ...
    if (amount.compareTo(UserSession.getInstance().getBalance()) > 0) {
      AlertHelper.showError(
          "Số dư không đủ",
          "Giá đặt không được vượt quá số dư hiện tại. Hãy nạp thêm tiền trước khi đấu giá.");
      return;
    }
      ...
      if (amount.compareTo(minimumAcceptableBid) < 0) {
        AlertHelper.showError(
            ...
            "Giá đặt tối thiểu là "
                + minimumAcceptableBid.toPlainString()
                + " (giá hiện tại + bước giá "
                + minBidStep.toPlainString()
                + ").");
        return;
      }
    }
  ...
  private void updatePlaceBidButtonState() {
    ...
    try {
      ...
      boolean lowerThanMinimum =
          minimumAcceptableBid != null && value.compareTo(minimumAcceptableBid) < 0;
  ...
  public static final class BidHistoryRow {
    ...
    private BidHistoryRow(
        long bidId, String timeText, String bidderText, String amountText, String winningText) {
      this.bidId = bidId;
    ...
    static BidHistoryRow fromDto(BidResultDto dto) {
      ...
      String bidder =
          dto.getBidderUsername() == null || dto.getBidderUsername().isBlank()
              ? "#" + dto.getBidderId()
