 ...
 *
 * <p>// 1. Khai báo map (giữ thứ tự bằng LinkedHashMap) để tránh việc phải lặp code private final
 * Map<Control, Label> fieldErrorMap = new LinkedHashMap<>();
 *
 * <p>// 2. Gắn trong initialize() FormHelper.register(fieldErrorMap, emailField, emailError,
 * passwordField, passwordError ); FormHelper.bindClearOnChange(fieldErrorMap);
 *
 * <p>// 3. Khi submit FormHelper.clearAll(fieldErrorMap); ValidationResult result =
 * validator.validate(request); if (!result.valid()) { Map<String, Control> fieldMap =
 * Map.of("email", emailField, ...); FormHelper.applyErrors(result, fieldMap, fieldErrorMap); }
 */
public class FormHelper {
   ...
   *
   * <p>/* Hàm applyError nhận (ValidationResult, Map field: Control của các Field,Map control của
   * các Field : Label FieldError và kiểm tra nếu có lỗi thì show lỗi -> Có thể tái sử dụng, không
   * bị lặp if (cách tốt khi refractor code, dễ mở rộng,...-> thỏa mãn các nguyên lý)
   */
  public static void applyErrors(
      ValidationResult result, Map<String, Control> fieldMap, Map<Control, Label> errorMap) {
  ...
  /**
   * Lưu ý: Các tham số Control Field của các hàm trong class này sẽ đều là Control để có thể xử lý
   * trường hợp DatePicker Nó sẽ kiểm tra có phải là datePicker hay không, Nếu có thì DownCast nó
   * thành DatePicker và gọi hàm lấy node Text của nó.
   */
  ...
  /**
   * Xóa lỗi cho tất cả field trong map. forEach của Map nhận BiConsumer<K, V> nên viết được cả 2
   * cách: // Method reference fieldErrorMap.forEach(FormHelper::clearError); // Lambda tương đương
   * fieldErrorMap.forEach((field, label) -> FormHelper.clearError(field, label)); BiConsumer<T, U>
   * yêu cầu một function nhận 2 tham số, không trả về gì:
   */
  ...
  /**
   * Gắn listener vào tất cả field: khi user gõ/chọn → tự clear error. Gọi một lần trong
   * initialize() sau khi register() xong.
   */
  public static void bindClearOnChange(Map<Control, Label> fieldErrorMap) {
    fieldErrorMap.forEach(
        (field, errorLabel) -> {
          if (field instanceof DatePicker dp) {
            dp.valueProperty().addListener((o, old, val) -> clearError(field, errorLabel));
          } else {
  ...
  /**
   * Gắn textProperty listener nếu field có text (TextField, PasswordField). Dùng pattern matching
   * để tránh import javafx.scene.control.TextInputControl.
   */
  public static void tryBindTextChange(Control field, Label errorLabel) {
    if (field instanceof javafx.scene.control.TextInputControl tic) {
      tic.textProperty().addListener((o, old, val) -> clearError(field, errorLabel));
    }
  }
}
